...
6. Run quality checks: `npm run build` (must pass with no errors)
...
- Example: Export types from db.ts for UI components
```
...
- "Components use Tailwind CSS with teal as primary color"
...
3. Note any visual changes in the progress log
...
- Update prd.json to mark `passes: true` after successful implementation
